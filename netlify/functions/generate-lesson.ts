...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const system = `Expert teacher. ${formatProfile(profile)} Respond ONLY with a valid JSON object. Do not wrap it in markdown code fences. (Inline Markdown emphasis inside the string values is expected and encouraged.)`;

  const userPrompt = `Teach "${nodeTitle}" (from the "${pathTitle}" path). Return ONLY: { "lessonText":"3-4 sentence core explanation for this learner", "example":"One short concrete worked example or real scenario (2-3 sentences)", "diagram":{"type":"flow|cycle|comparison|pyramid (pick what fits the concept best)","title":"diagram caption","items":[{"label":"max 4 words","detail":"max 8 words"}],"colA":"only for comparison","colB":"only for comparison"}, "keyTerms":[{"term":"word","def":"max 10 words"} x3], "funFact":"one surprising fact", "quiz":[{"q":"one check question","options":["A","B","C"],"answer":"exact correct option"}] } "quiz" must be a JSON array even though it has only one item. In "lessonText", "example" and "funFact", use light Markdown to make reading engaging (keep it inside the JSON string values): **bold** for the key concepts a learner must remember, *italics* for emphasis, \`backticks\` for technical/foreign words, specific terms, code or exact values, and ==highlight== for the single most important takeaway. Use them naturally and sparingly — do not over-format. NEVER use underscores for emphasis (asterisks only). Do NOT use Markdown headings, lists or line breaks; plain sentences only, just with inline emphasis. Diagram rules: flow=3-5 sequential steps; cycle=3-5 repeating stages; comparison=2x2-3 items contrasting colA vs colB (items: first half colA, second half colB); pyramid=3-4 levels broad-to-narrow.`;
